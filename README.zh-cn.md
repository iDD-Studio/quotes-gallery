...
```bash
# 在项目根目录直接启动开发服务器
hugo server --disableFastRender -D --port 1313
```
...
3. **实时预览**
   ...
   修改 `content/` 目录下的内容文件，或 `static/quotes-images/` 下的图片，浏览器会自动刷新显示更改。

4. **多语言**
   
   当前支持：`http://localhost:1313/`（默认）、`http://localhost:1313/zh-cn/`

### 开发提示

- **主题开发**：使用 `npm run dev:theme` 获得更详细的调试信息
- **内容测试**：在 `content/` 目录下添加或修改内容进行测试
- **画廊图片**：将图片放入 `static/quotes-images/` 即可在画廊中显示
- **样式修改**：修改 `assets/css/` 目录下的样式文件
- **配置**：站点配置在根目录 `hugo.yaml`、`hugo.toml`
